...
@forelse($goals as $goal)
    <div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03] p-6 mb-4">
        <div class="flex items-start justify-between mb-4">
            ...
            <div class="flex items-center gap-1">
                ...
                <form action="{{ route('goals.destroy', $goal) }}" method="POST"
                      x-data x-on:submit.prevent="if(confirm('{{ __('goals.confirm_delete_goal') }}')) $el.submit()">
                    @csrf
                    <button type="submit"
        ...
        <div class="mb-3">
            <div class="w-full h-3 bg-gray-100 dark:bg-gray-800 rounded-full overflow-hidden">
                <div class="h-3 rounded-full transition-all duration-500
                    {{ $goal->progress_pct >= 100 ? 'bg-green-500' : 'bg-brand-500' }}"
        ...
        <div class="flex items-center justify-between">
            <span class="text-sm text-gray-500 dark:text-gray-400">
                ...
                <span class="text-gray-300 dark:text-gray-600">/ {{ number_format($goal->target_amount, 2, ',', '.') }} {{ $currency }}</span>
            </span>
            <div class="flex items-center gap-3">
                <span class="font-bold text-sm {{ $goal->progress_pct >= 100 ? 'text-green-500' : 'text-brand-500' }}">
                    {{ $goal->progress_pct }}%
                </span>
