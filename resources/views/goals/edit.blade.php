...
<div class="mb-6">
    <x-common.page-breadcrumb :pageTitle="__('goals.edit_goal')" />
</div>

<div class="max-w-2xl">
    <div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
        <div class="px-6 py-5 border-b border-gray-100 dark:border-gray-800">
            <h3 class="text-base font-semibold text-gray-800 dark:text-white/90">{{ __('goals.edit_goal') }}</h3>
        </div>
        <div class="p-6">
            <form action="{{ route('goals.update', $goal) }}" method="POST">
                ...
                <div class="mb-5">
                    <div class="flex gap-2 mb-1.5">
                        ...
                        <label class="text-sm font-medium text-gray-700 dark:text-gray-400 flex-1">{{ __('goals.goal_name') }} <span class="text-red-500">*</span></label>
                    </div>
                    <div class="flex gap-3">
                        <input type="text" name="emoji" maxlength="4"
                               value="{{ old('emoji', $goal->emoji) }}"
                        ...
                        <input type="text" name="name" required
                               ...
                               placeholder="{{ __('goals.goal_name_placeholder') }}"
                ...
                <div class="mb-5">
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-400 mb-1.5">
                        {{ __('goals.goal_target') }} <span class="text-red-500">*</span>
                    </label>
                    <div class="relative">
                ...
                <div class="mb-8">
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-400 mb-1.5">
                        {{ __('goals.goal_date') }}
                        <span class="text-gray-400 font-normal ml-1">({{ __('goals.optional') }})</span>
                    </label>
                    <input type="date" name="target_date"
                           value="{{ old('target_date', $goal->target_date ? \Carbon\Carbon::parse($goal->target_date)->format('Y-m-d') : '') }}"
