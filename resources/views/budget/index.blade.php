...
<div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
    <div>
        <h1 class="text-2xl font-bold text-gray-800 dark:text-white/90">{{ __('ui.nav_budget') }}</h1>
    ...
    <table class="w-full text-sm">
        <thead>
            <tr class="border-b border-gray-100 dark:border-gray-800">
                ...
                <th class="px-6 py-3 text-right text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">{{ __('ui.spent') }}</th>
                            ...
                            <div>
                                ...
                                @if($category->rollover != 0)
                                    <div class="text-xs {{ $category->rollover > 0 ? 'text-green-500' : 'text-red-400' }}">
                                        {{ $category->rollover > 0 ? '+' : '' }}{{ number_format($category->rollover, 2, ',', '.') }} {{ __('ui.rollover') }}
                                    </div>
                                @endif
                            </div>
                        </div>
